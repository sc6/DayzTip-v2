...
//Outputs:
//	Writes client's comment tip to database
//	Gives the gold reward
//	Reports for possible injection attempts


if(!isset($_POST['comment'])) { //only display title when NOT submitted ?>
	<br />
	<span class="subsubtitle">Submit a comment</span>
...
if(isset($_POST['comment'])) {	//after the client submits the form with their comment...
	...
	$query = "INSERT INTO comments (comment, item, username, type) VALUES ('".$sanitized_comment."', '".$_SESSION['item']."', '".$username."', 'other')";
	mysqli_query($link, $query);
	
	//gives the gold
	$query = "UPDATE user_stats SET gold = gold + ".$comment_reward." WHERE username='".$username."'";
	mysqli_query($link, $query);
...
?>
		...
		<br /><br />
		• <strong>You</strong> will be rewarded <strong><?php echo $comment_reward ?> gold</strong> for each comment submission, plus any points that are given to your comment.
		<br />
		• Use this section to explain any other concerns pertaining to this item or article.
		<br />
		• Any off-topic submission will be removed.
	
	<div class="small_break"></div>
	<form method="POST">
		<textarea name="comment" class="fancy" required="required" maxlength="150"></textarea><br /><br />
		<input type="submit" value="Submit comment (+<?php echo $comment_reward ?> gold)" />
	</form>
