...
//Outputs:
//	Writes client's condition tip to database
//	Gives the gold reward
//	Reports for possible injection attempts


if(!isset($_POST['condition'])) { //only display title when NOT submitted ?>
	<br />
	<span class="subsubtitle">Edit: Condition Effects</span>
...
if(isset($_POST['condition'])) {	//after the client submits the form with their comment...
	
	$raw_comment = $_POST['condition'];
	...
	$query = "INSERT INTO comments (comment, item, username, type) VALUES ('".$sanitized_comment."', '".$_SESSION['item']."', '".$username."', 'condition')";
	mysqli_query($link, $query);
	
	//gives the gold
	$query = "UPDATE user_stats SET gold = gold + ".$condition_reward." WHERE username='".$username."'";
	mysqli_query($link, $query);
...
?>
		...
		<br /><br />
		• <strong>You</strong> will be rewarded <strong><?php echo $condition_reward ?> gold</strong> for each correct submission.
		<br />
		• Use this section to explain any effects a condition can have on this item.
		<br />
	...
	<form method="POST" action="">
		<textarea name="condition" class="fancy" required="required" maxlength="150"></textarea><br /><br />
		<input type="submit" value="Submit tip (+<?php echo $condition_reward ?> gold)" />
	</form>
