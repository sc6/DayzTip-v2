...
//Outputs:
//	Writes client's options tip to database
//	Gives the gold reward
//	Reports for possible injection attempts


if(!isset($_POST['options'])) { //only display title when NOT submitted ?>
	<br />
	<span class="subsubtitle">Edit: Item options</span>
...
if(isset($_POST['options'])) {	//after the client submits the form with their comment...
	
	$raw_comment = $_POST['options'];
	...
	$query = "INSERT INTO comments (comment, item, username, type) VALUES ('".$sanitized_comment."', '".$_SESSION['item']."', '".$username."', 'options')";
	mysqli_query($link, $query);
	
	//gives the gold
	$query = "UPDATE user_stats SET gold = gold + ".$options_reward." WHERE username='".$username."'";
	mysqli_query($link, $query);
...
?>
		...
		<br /><br />
		• <strong>You</strong> will be rewarded <strong><?php echo $options_reward ?> gold</strong> for each correct submission.
		<br />
		• Use this section to write in verbatim (word-for-word) the in-game options of the item.
		<br />
	...
	<form method="POST" action="">
		<textarea name="options" class="fancy" required="required" maxlength="100"></textarea><br /><br />
		<input type="submit" value="Submit tip (+<?php echo $options_reward ?> gold)" />
	</form>
