...
//Outputs:
//	Writes client's uses tip to database
//	Gives the gold reward
//	Reports for possible injection attempts


if(!isset($_POST['uses'])) { //only display title when NOT submitted ?>
	<br />
	<span class="subsubtitle">Edit: Item uses</span>
...
if(isset($_POST['uses'])) {	//after the client submits the form with their comment...
	
	$raw_comment = $_POST['uses'];
	...
	$query = "INSERT INTO comments (comment, item, username, type) VALUES ('".$sanitized_comment."', '".$_SESSION['item']."', '".$username."', 'uses')";
	mysqli_query($link, $query);
	
	//gives the gold
	$query = "UPDATE user_stats SET gold = gold + ".$uses_reward." WHERE username='".$username."'";
	mysqli_query($link, $query);
...
?>
		...
		<br /><br />
		• <strong>You</strong> will be rewarded <strong><?php echo $uses_reward ?> gold</strong> for each correct submission.
		<br />
		• Use this section to explain what this item is used for.
		<br />
	...
	<form method="POST" action="">
		<textarea name="uses" class="fancy" required="required" maxlength="150"></textarea><br /><br />
		<input type="submit" value="Submit tip (+<?php echo $uses_reward ?> gold)" />
	</form>
